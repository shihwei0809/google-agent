...
def create_pptx(pptx_path):
    ...
    bullets_upd4 = [
        "獨立的月度 24 小時記錄分頁：雲端 Apps Script 重構日誌寫入路由，日常每 10 分鐘心跳與溫度觀測，自動分流寫入「24小時紀錄_YYYY-MM」分頁，與通報紀錄統一採用西元命名，完美防範跨年度名稱衝突。",
        "專屬綠色系排版風格：針對「24小時記錄」分頁套用專屬的深綠色主題表頭（#2E7D32）與排版格式，利於視覺區分，畫面更乾淨整潔。",
        "警報標題格式優化：移除了警報內文前的試算表分頁名稱字串，使 LINE/Email 通報標題更直接美觀。"
    ]
