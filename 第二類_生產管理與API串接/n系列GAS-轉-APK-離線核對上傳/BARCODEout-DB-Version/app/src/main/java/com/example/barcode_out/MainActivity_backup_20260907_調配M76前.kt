...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSync.setOnClickListener {
            ...
            Thread {
                try {
                    NetworkHelper.uploadToDatabase(dbHelper)
                    runOnUiThread {
                        updateStatusText()
                    ...
                    runOnUiThread {
                        btnSync.isEnabled = true
                        Toast.makeText(this@MainActivity, "⚠️ 同步失敗，請檢查網路或後端伺服器設定", Toast.LENGTH_LONG).show()
                    }
                }
            }.start()
        }
    ...
    // 【2026-09-07 更新註記】：檢查 1 系列格式 (相容 20 碼舊庫存 & 24 碼新標籤，放行 TS 與 TW)
    private fun check1SeriesFormat(code: String?): String {
        val s = (code ?: "").trim()
        if (s.startsWith("1")) {
            val len = s.length
            // 1. 長度驗證：相容 20 碼舊庫存 & 24 碼新標籤
            if (len != 20 && len != 24) return "❌ 格式錯誤！\n👉 [1開頭] 長度需 20 碼或 24 碼 (目前 ${len})"
            val suffix = s.takeLast(2).uppercase()
            // 2. 結尾代碼：同時放行 TSMC 常見的 TS 與 Email 說明的 TW
    ...
    private fun verifyPairStrict(scanVal: String, masterVal: String, localLabel: String, masterLabel: String): VerifyResult {
        ...
        // 【2026-09-07 更新註記】：1 系列單據比對同步支援 20/24 碼與 TS/TW 結尾
        val scanUpper = scan.uppercase()
        val is1Series = scan.startsWith("1") && 
                        (scan.length == 20 || scan.length == 24) && 
                        (scanUpper.endsWith("TS") || scanUpper.endsWith("TW"))
