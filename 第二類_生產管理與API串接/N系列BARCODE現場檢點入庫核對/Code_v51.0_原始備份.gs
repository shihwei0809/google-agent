// ==========================================
// Google Apps Script v51.0 (AZ欄位順序自動修正版) - 原始未修改備份
// 說明：本版本為修改支援 24 碼前之原始存檔，首碼 1 僅支援 20 碼。
// ==========================================

function doGet(e) {
  if (e.parameter && e.parameter.page == 'query') {
    return HtmlService.createHtmlOutputFromFile('Query')
        .setTitle('N系列現場作業紀錄查詢系統')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
        .addMetaTag('viewport', 'width=device-width, initial-scale=1');
  }
  return HtmlService.createHtmlOutputFromFile('Index')
      .setTitle('N系列BARCODE現場作業檢點 (v51.0)')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
      .addMetaTag('viewport', 'width=device-width, initial-scale=1');
}
...
function processAndSave(data) {
  ...
  var boxBarcode = toStr(f[0]); var boxMat = ""; var boxExpiry = ""; 

  if (boxBarcode !== "") {
    var len = boxBarcode.length, prefix = boxBarcode.charAt(0), suffix = boxBarcode.slice(-2); 
    if (suffix !== "TS") errorList.push('❌ [料號與保存期限條碼] 格式錯誤！結尾必須是 TS。');
    else if (prefix === "1") { if (len !== 20) errorList.push('❌ [料號與保存期限條碼] 長度錯誤！首碼1應為20碼。<br>目前長度: ' + len); else { var rD = boxBarcode.substring(10, 18); if (/^\d{8}$/.test(rD)) { boxExpiry = rD; boxMat = boxBarcode.substring(1, 10).trim(); } else errorList.push('❌ 日期解析失敗'); } } 
    else if (prefix === "7") { if (len !== 29) errorList.push('❌ [料號與保存期限條碼] 長度錯誤！首碼7應為29碼。<br>目前長度: ' + len); else if (boxBarcode.indexOf("-T0") === -1) errorList.push('❌ 格式錯誤！首碼為7時，必須包含 "-T0"'); else { var rD = boxBarcode.substring(19, 27); if (/^\d{8}$/.test(rD)) { boxExpiry = rD; boxMat = boxBarcode.substring(0, 19).trim(); } else errorList.push('❌ 日期解析失敗'); } } 
    else errorList.push('❌ 首碼須為 1 或 7');
  }
...
function searchRecords(params) {
  ...
  for (var i = 0; i < targetSheets.length; i++) {
    ...
    var hE = toStr(header[4]); 
    var hF = toStr(header[5]); 
    ...
    for (var r = 0; r < data.length; r++) {
      ...
      if (isMatch) {
        ...
        var boxMat = "";
        if (boxStr.startsWith("1") && boxStr.length>=10) boxMat = boxStr.substring(1,10);
        else if (boxStr.startsWith("7") && boxStr.length>=19) boxMat = boxStr.substring(0,19);
