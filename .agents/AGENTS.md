...
1. **主動詢問開發狀態 (Git 分支防呆判斷)**：
   - AI 必須使用多選題或直接提供編號選項詢問：「*今天的進度都已經在測試環境驗證成功、準備正式上線了嗎？還是只是要備份跨機開發的進度？*」
   - **[1] 狀況 A（未完工/純備份）**：若使用者回覆 1，AI 必須建立或保留在 `feat/[當天日期]`（例如 `feat/20260825`）等開發分支，將修改 Commit 並 Push 到該分支。**絕對禁止**直接推送到 `main`。
   - **[2] 狀況 B（驗收完畢/正式上線）**：若使用者回覆 2，AI 必須將目前的開發進度合併至 `main`，並**自動執行「發布版本」流程（自動偵測現有 Tag 往上加 1、建立新 Tag，並連同 `main` 一起 Push 上雲端）**，確保 `main` 永遠是乾淨可用的正式版且具備最新版號。
...
## 5. 主動開工偵測與全分支智慧同步規則 (跨機防呆同步)
當 AI 接收到使用者的第一句話，且判定為全新的對話或一天的開始時：
1. **主動攔截與強制同步**：即使使用者沒有說「開工」而是直接給予開發指令，AI 必須**優先暫停任務**並回覆：「*為了避免跨電腦進度不同步，我先為您執行『開工同步』，完成後會立刻執行您的任務。*」
2. **背景執行全分支智慧同步標準程序（主專案 + 中央大腦全覆蓋）**：
   - 【同步主專案】：在專案根目錄執行 `git fetch --all --tags` 抓取遠端所有主支與分支的最新狀態。
   - **智慧偵測遠端最新開發分支**：檢查是否有遠端最新的 `origin/feat/YYYYMMDD` 或 `origin/dev` 分支，若該分支有比本機更新的進度（即另一台電腦收工時推送到分支的進度），**AI 必須自動將該分支合併/同步至本機工作目錄**，確保主支與分支都不漏抓！
   - 同步執行 `git pull origin main` 確保主支最新。
   - **【同步中央大腦】**：切換至 `%USERPROFILE%\.ai` 並執行 `git pull origin main`，確保跨電腦的 AI 全域規則與技能庫也保持在最新狀態。
   - 讀取專案或根目錄的 `HANDOVER.md`，並**將 `HANDOVER.md` 內的「前次進度與交接事項」以 Markdown `text` 區塊完整顯示給使用者看**，確保雙方對齊開發狀態，避免踩到跨機歷史陷阱。
