# 雲端執行與 HMI 部署指引：使用 Google Apps Script 實現 24 小時免開機與管理網頁 (v4.1)

如果希望系統在**電腦關機時也能自動運作**，最完美且 **100% 免費** 的做法是將程式碼直接部署在您的 **Google 試算表雲端腳本 (Google Apps Script)** 中。

本系統自 v4.1 起引進了全新的 **HMI 雲端人機介面管理網頁手機優化與密碼安全防護版**，管理人員不需直接連進 Google 試算表，只需開啟瀏覽網址，即可在精美的深色科技風網頁中即時監看溫度、修改設定、管理通報名冊並查詢歷史紀錄。

---

## 🔑 重要安全性提示：管理網頁密碼
*   **預設密碼**：本系統 v4.1 版本預設管理密碼為 **`admin888`**。
*   **密碼保護範圍**：為了防止未授權的人員隨意變更通報名冊或閾值，凡是點選「系統設定」、「聯絡人名冊」或「快速工具與功能測試」時，均需要輸入管理密碼進行身份驗證。
*   **修改密碼**：您可以直接在 Google 試算表的 **`系統設定` 工作表第 5 列**（管理網頁密碼）進行修改；或者在 HMI 設定網頁輸入新密碼儲存，系統將會自動套用新密碼。

---

## 📱 行動裝置介面適配 (手機版優化)
*   **底部導覽列**：在手機或窄螢幕（寬度 <= 768px）下，左側側邊欄會自動隱藏，轉換為大拇指便於點選的**底部懸浮導覽列**。
*   **橫向滑動表格**：為防止聯絡人信箱、LINE ID 以及紀錄時間在窄螢幕上被截斷或強制換行擠壓，表格加入了 **min-width 與水平捲動 (Swipe-scroll)** 設計，可透過橫向滑動檢視完整資訊。
*   **浮動通知**：右下角通知（Toast）會自動上移，避免被手機底部導覽列擋住。
...
3. 進行設定：
   - **說明 (Description)**：輸入 `溫度通報系統 HMI v4.1`。
...
本系統自 v4.1 起，新增了**自動同步調整排程**功能，您無須手動配置排程，直接透過選單或 HMI 網頁一鍵完成：
...
- **快速工具**：一鍵觸發強制發送測試、清除狀態鎖定、重設欄寬（需要密碼解鎖）。
...
- **管理密碼修改**：提供密碼輸入框，輸入新密碼以進行修改（不修改請留空）。
- **儲存設定**：儲存後，雲端試算表設定檔將同步更新，Apps Script 時間排程亦會自動變更。
...
- **一鍵匯出 CSV**：點選「匯出本頁 CSV」即可將目前篩選後的紀錄下載為 Excel 相容 CSV 檔案。
