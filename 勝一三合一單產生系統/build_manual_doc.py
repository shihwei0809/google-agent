...
def create_manual_docx(docx_path):
    ...
    bullets = [
        ...
        ("3. 多產品品名動態辨識與固定重量自動對照：", "自動辨識勝一多樣產品品名（如 SEP73E5, PMAHQ, SEP73E4, CPNE4R, IPA 等），單列生產履歷 (Chemical_Lorry)卡片標題自動動態顯示 Shiny [品名] Lorry，並由系統內建對照表自動填入對應產品之固定充填重量（如 4300 KG），不需人員手動輸入重量。"),
        ...
        ("6. 既有通知表一鍵還原與併排修正：", "支援載入既有單列生產履歷 (Chemical_Lorry)還原原始列位，填寫修正到廠時間後自動於右側併排產生紅字加粗與刪除線之出貨修正卡片。")
    ]
    ...
    steps = [
        ...
        (
            ...
            "點擊「從排程匯入」選取『勝一訂單.xlsx』時，系統會自動過濾非台積電廠商，並優先鎖定「當天至+2天 (今天~後天)」的最新台積電排程自動預選！亦可透過快捷按鈕單獨切換「今天」、「明天」、「後天」或倒數筆數。",
            "image_shinyi_import.png",
            "▲ 圖 5：勝一訂單台積電專屬過濾與當天至+2天優先推薦示意圖"
        ),
        (
            "【步驟 6】還原既有通知表與點對點修正到廠時間",
            "當下午需要修正上午產生的通知表時，點擊「載入既有『單列生產履歷 (Chemical_Lorry)』修訂」（預設開啟當天資料夾）。一鍵還原所有原始列號與 10~11 碼批號，只需在該列輸入「修正到廠時間」即可產出併排修正卡片！",
            "image5.png",
    ...
    rows_data = [
        ...
        ("單列生產履歷 (Chemical_Lorry)", "單列生產履歷 (Chemical_Lorry).xlsx", "卡片標題與左側品名依產品動態調整（如 Shiny SEP73E5 Lorry），充填數量自動帶入固定重量；若有修正時間自動併排呈現修正卡片"),
...
if __name__ == "__main__":
    docx_file = os.path.join(base_dir, "勝一三合一單產生系統_操作手冊_V2.docx")
    pdf_file = os.path.join(base_dir, "勝一三合一單產生系統_操作手冊_V2.pdf")
