...
def auto_select_device(preferred_device: Optional[str] = "cuda") -> str:
    ...
    if preferred.startswith("cuda") and torch.cuda.is_available():
        return preferred
    if preferred.startswith("xpu") and _has_xpu():
        return preferred
...
def resolve_runtime_device(device: Optional[str], configured_device: str = "cuda") -> str:
    ...
    if explicit.startswith("cuda"):
        ...
        return explicit
    if explicit.startswith("xpu"):
        if not _has_xpu():
            raise ValueError(
                f"Requested device '{device}', but XPU is not available. "
                "Use device='auto' for automatic fallback."
            )
        return explicit
    if explicit == "mps":
    ...
    raise ValueError(
        ...
        "'cuda', 'xpu', or indexed devices like 'cuda:0' / 'xpu:0'."
    )
