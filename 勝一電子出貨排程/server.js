...
app.post('/api/admin/reset', (req, res) => {
  try {
    ...
    if (role !== 'sales' && role !== 'tech_manager') {
      return res.status(403).json({ success: false, message: '權限不足！只有管理者可以重置資料庫。' });
    }
    const db = getDB();
    db.orders = [];
    db.logs = [];
    
    // Reload users and drivers from Excel
    const excelUsers = loadUsersFromExcel();
    const excelDrivers = loadDriversFromExcel();
    if (excelUsers) db.users = excelUsers;
    if (excelDrivers) db.drivers = excelDrivers;
    
    addLog(db, username || '管理者', role, '重置資料庫', '清空所有出貨訂單與作業日誌');
    saveDB(db);
    res.json({ success: true, message: '系統資料已成功重置與清空！' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
// Admin Restore API
app.post('/api/admin/restore', upload.single('file'), (req, res) => {
  try {
    const username = req.query.operator || '管理者';
    const role = req.query.role;
    if (role !== 'sales' && role !== 'tech_manager') {
      return res.status(403).json({ success: false, message: '權限不足！' });
    }
    if (!req.file) {
      return res.status(400).json({ success: false, message: '請上傳備份檔案' });
    }
    ...
    if (backupData.logs) db.logs = backupData.logs;
    if (backupData.drivers) db.drivers = backupData.drivers;
    if (backupData.users) db.users = backupData.users;
    
    addLog(db, username, role, '備份還原', '成功自備份檔案還原系統資料');
    saveDB(db);
    res.json({ success: true, message: '系統資料已成功還原！' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: `還原失敗: ${err.message}` });
  }
});
