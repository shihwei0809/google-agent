...
# Restore backup
shutil.copy(r'D:\GOOGLE ANGET\三合一單自動產生器\main.py.bak', r'D:\GOOGLE ANGET\三合一單自動產生器\main.py')
...
# Let's fix duplicated lines FIRST
def remove_duplicates(text):
    lines = text.split('\n')
    new_lines = []
    for line in lines:
        if new_lines and new_lines[-1] == line and 'set("")' in line:
            continue
...
# 4. clear
content = re.sub(r'\n( {16})if "po_var" in entry: entry\["po_var"\]\.set\(""\)', r'\n\1if "po_var" in entry: entry["po_var"].set("")\n\1if "origin_var" in entry: entry["origin_var"].set("")\n\1if "part_var" in entry: entry["part_var"].set("")', content)
content = re.sub(r'\n( {12})if "po_var" in entry: entry\["po_var"\]\.set\(""\)', r'\n\1if "po_var" in entry: entry["po_var"].set("")\n\1if "origin_var" in entry: entry["origin_var"].set("")\n\1if "part_var" in entry: entry["part_var"].set("")', content)

# 5. update_part_no and bind trace
...
# For on_paste
content = content.replace('''elif parsed["loc"]:
                            self.entries[curr_row]["loc_var"].set(parsed["loc"])
                        elif parsed["date"]:''', '''elif parsed["loc"]:
                            self.entries[curr_row]["loc_var"].set(parsed["loc"])
                            if parsed.get("origin"):
                                self.entries[curr_row]["origin_var"].set(parsed["origin"])
                        elif parsed["date"]:''')

content = content.replace('''if parsed["loc"]:
                        self.entries[curr_row]["loc_var"].set(parsed["loc"])
                    if parsed["date"]:''', '''if parsed["loc"]:
                        self.entries[curr_row]["loc_var"].set(parsed["loc"])
                    if parsed.get("origin"):
                        self.entries[curr_row]["origin_var"].set(parsed["origin"])
                    if parsed["date"]:''')
