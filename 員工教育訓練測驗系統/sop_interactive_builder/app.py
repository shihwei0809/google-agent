...
VOICE_CLONER_DIR = Path("C:/GOOGLE ANGET/ai-voice-cloner-guide")
...
async def get_voices():
    # 動態讀取本機克隆語音
    clone_voices = []
    clone_voices_dir = VOICE_CLONER_DIR / "voices"
    if clone_voices_dir.exists() and clone_voices_dir.is_dir():
        for d in clone_voices_dir.iterdir():
            if d.is_dir():
                clone_voices.append({"id": d.name, "label": f"{d.name} (本機克隆)"})
...
async def extract_document(
    ...
    if extract_mode == "local":
        ...
        page_hint = f"大約 {slide_count} 頁的" if slide_count and slide_count.isdigit() and int(slide_count) > 0 else "多頁的"
        prompt = (
            f"請將以下教育訓練教材內容整理成{page_hint}『簡報腳本』，並根據內容生成 3 到 5 題『選擇題測驗』。\n"
...
async def generate_audio(
    ...
    audio_ext = "wav" if tts_engine in ["gemini", "cloner"] else "mp3"
    audio_path = job_dir / f"audio_{page_index}.{audio_ext}"
    
    logger.info(f"")
    logger.info(f"======== 準備合成語音 [第 {page_index + 1} 頁] ========")
    logger.info(f"引擎: {tts_engine} | 語音角色: {voice} | 儲存路徑: {audio_path.name}")

    try:
        if tts_engine == "gemini":
            logger.info("⏳ 呼叫 Gemini TTS 雲端合成中...")
            keys = parse_api_keys(api_keys) or parse_api_keys(os.environ.get("GEMINI_API_KEYS", ""))
            if not keys:
            ...
            for key in keys:
                try:
                    ...
                    continue
            if not success:
                raise ValueError("所有金鑰皆失效或發生錯誤")
        
        elif tts_engine == "cloner":
            clone_script = VOICE_CLONER_DIR / "clone.py"
            if not clone_script.exists():
                raise HTTPException(status_code=500, detail="找不到克隆語音腳本 clone.py")
            
            venv_python = VOICE_CLONER_DIR / ".venv" / "Scripts" / "python.exe"
            python_exe = str(venv_python) if venv_python.exists() else "python"
            
            logger.info(f"⏳ 正在呼叫本機克隆引擎運算中 (這可能需要數分鐘的時間)...")
            cmd = [python_exe, str(clone_script), text, "--voice", voice, "--output", str(audio_path)]
            process = subprocess.run(cmd, cwd=str(VOICE_CLONER_DIR), capture_output=True, text=True, encoding='utf-8')
            
            if process.returncode != 0:
                logger.error(f"Clone TTS Error: {process.stderr}")
                raise HTTPException(status_code=500, detail=f"克隆語音生成失敗: {process.stderr}")
            
            logger.info(f"✅ 克隆語音合成完成！(第 {page_index + 1} 頁)")
            
            if not audio_path.exists():
                raise HTTPException(status_code=500, detail="克隆腳本執行成功，但未產生預期的音檔。")
        
        else:
            # 預設 Edge TTS
            logger.info("⏳ 呼叫 Edge TTS 雲端合成中...")
            communicate = edge_tts.Communicate(text, voice)
            await communicate.save(str(audio_path))
            
        logger.info(f"🎉 語音處理結束 (第 {page_index + 1} 頁)")
        import time
...
async def export_pptx(job_id: str = Form(...)):
    ...
    # Define colors
    primary_color = RGBColor(15, 23, 42)    # Slate 900 (Dark Blue)
    accent_color = RGBColor(56, 189, 248)   # Sky 400 (Light Blue)
    text_color = RGBColor(51, 65, 85)       # Slate 700 (Dark Gray)
    ...
    for slide_data in job_data.get("slides", []):
        ...
        fill.solid()
        fill.fore_color.rgb = RGBColor(248, 250, 252) # 淺灰背景
        
        # --- 美化標題 ---
        title = slide.shapes.title
        title.text = coerce_str(slide_data.get("title", ""))
        
        # 在標題後方加上色塊
        if title.has_text_frame:
            title.fill.solid()
            title.fill.fore_color.rgb = primary_color
        ...
        content.text = coerce_str(slide_data.get("content", ""))
        ...
        notes_slide.notes_text_frame.text = coerce_str(slide_data.get("narration", ""))
...
async def export_web(job_id: str = Form(...)):
    ...
    for i, s in enumerate(job_data.get("slides", [])):
        audio_ext = "wav" if job_data.get("tts_engine") in ["gemini", "cloner"] else "mp3"
