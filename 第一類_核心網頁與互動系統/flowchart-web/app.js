...
function calculatePath(fromId, toId, points) {
  ...
  if (fromG && toG) {
    ...
    if (fromG.id === "raw-group" && toG.id.startsWith("process-block")) {
      startX = fromG.x + fromG.w;
      startY = toG.y + toG.h/2;
      endX = toG.x;
      endY = toG.y + toG.h/2;
    ...
    } else if (fromId === "raw-group" && (toId === "process-block-s3" || toId === "process-block-s5")) {
...
function updateSelectedNodeUI() {
  ...
  if (selectedItem.type === 'node' || selectedItem.type === 'group') {
    ...
    if (editConnSection) {
      ...
      if (gc) {
        document.getElementById("editConnFromText").textContent = getReadableName(gc.from);
        document.getElementById("editConnToText").textContent = getReadableName(gc.to);
...
function updateConnectionEditorUI() {
  ...
  if (isEditingMode) {
    ...
    data.groupConnections.forEach((conn, index) => {
      ...
      const fromName = getReadableName(conn.from).replace(/^\[.*?\]\s*/, '');
      const toName = getReadableName(conn.to).replace(/^\[.*?\]\s*/, '');
