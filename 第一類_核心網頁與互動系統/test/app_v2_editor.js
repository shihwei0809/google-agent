...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch story from local API (Port 8001)
    fetch('http://127.0.0.1:8001/api/v1/story')
        .then(response => {
            if (!response.ok) throw new Error("HTTP error " + response.status);
            return response.json();
        })
        .then(data => {
            console.log("Loaded story dynamically from API");
            // Copy data to storyData
            Object.assign(storyData, data);
            renderMangaPages(storyData);
            setupEventListeners();
            setupSettingsEvents();
        })
        .catch(error => {
            console.warn("Failed to load story from API, using offline fallback:", error);
            renderMangaPages(storyData);
            setupEventListeners();
            setupSettingsEvents();
        });
            ...
            panel.dialogues.forEach((d) => {
                ...
                dialoguesHTML += `
                    <div class="dialogue-item" id="item-${d.id}" data-id="${d.id}" data-text="${d.text}" data-speaker="${d.speaker}">
                        ...
                        <div class="edit-dialogue-wrapper" style="display: flex; gap: 0.5rem; width: 100%; margin-top: 0.3rem; margin-bottom: 0.5rem;">
                            <input type="text" class="edit-dialogue-input" value="${d.text}">
                            <button class="save-tts-btn" data-id="${d.id}" data-speaker="${d.speaker}" title="儲存台詞並產生新語音">
                                <i class="fa-solid fa-cloud-arrow-up"></i> 儲存並配音
                            </button>
                        </div>
                    </div>
                `;
...
// ==========================================================
// 視覺化編輯器 - 儲存故事與動態配音邏輯 (Version 2)
// ==========================================================
document.addEventListener('click', (e) => {
    const btn = e.target.closest('.save-tts-btn');
    if (!btn) return;
    
    e.stopPropagation();
    const dId = btn.getAttribute('data-id');
    const speaker = btn.getAttribute('data-speaker');
    const itemNode = document.getElementById(`item-${dId}`);
    const inputEl = itemNode.querySelector('.edit-dialogue-input');
    ...
    // 1. 更新本機的記憶體資料
    findAndUpdateDialogue(dId, newText);
    
    // 2. 呼叫 PUT API 將更新後的故事寫回 story.json
    fetch('http://127.0.0.1:8001/api/v1/story', {
        method: 'PUT',
    ...
    .then(res => {
        if (!res.ok) throw new Error('儲存故事至 story.json 失敗');
        return res.json();
    })
    .then(() => {
        // 3. 呼叫 POST API 動態為該對話產生配音檔
        return fetch('http://127.0.0.1:8001/api/v1/generate-speech', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                text: newText,
    ...
    .then(res => {
        if (!res.ok) throw new Error('動態語音合成失敗');
        return res.json();
    })
    .then(result => {
        console.log("Success updating story and generating speech:", result);
        btn.innerHTML = '<i class="fa-solid fa-check"></i> 成功！';
        btn.style.backgroundColor = '#00a896';
        
        // 更新 DOM 節點的 data-text，這樣點擊播放時才會播放新台詞
        itemNode.setAttribute('data-text', newText);
    ...
    .catch(err => {
        console.error("Error in save & tts process:", err);
        alert('操作失敗: ' + err.message);
        btn.innerHTML = '<i class="fa-solid fa-xmark"></i> 失敗';
...
// 尋找並更新對話內容的輔助函式
function findAndUpdateDialogue(id, newText) {
    for (const page of storyData.pages) {
        for (const panel of page.panels) {
            for (const d of panel.dialogues) {
                if (d.id === id) {
