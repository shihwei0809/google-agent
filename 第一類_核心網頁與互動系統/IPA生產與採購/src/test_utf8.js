  ...
  function updateAllDropdowns() {
    ...
    prodSelects.forEach(select => {
      ...
      select.innerHTML = isFilter ? '<option value="all">全部產品</option>' : '';
  ...
  function setupEventListeners() {
    ...
    document.getElementById('btn-add-schedule').addEventListener('click', () => {
      ...
      if (state.tanks.filter(t => t.status === 'normal').length === 0) {
        alert('請先建立至少一個正常運作的儲槽/容器，才能規劃生產排程！');
        return;
      }
    ...
    document.getElementById('form-schedule').addEventListener('submit', (e) => {
      ...
      if (editingScheduleId) {
        ...
        state.schedules.push({ id, productId, tankId, sourceTankId, rate, start, end, yield: yieldVal });
      }
