    ...
    if os.path.exists(log_path):
        try:
            with open(log_path, "r", encoding="utf-8") as f:
                for line in f:
                    if "generate_narration_fish.py" in line:
                        ...
                        for tc in tool_calls:
                            ...
                            if isinstance(args, dict):
                                ...
                                if content and "generate_narration_fish" in str(args.get("TargetFile")):
                                    hits.append((folder, step.get("step_index"), tc.get("name"), content))
                                elif content and "generate_narration_fish" in content:
                                    hits.append((folder, step.get("step_index"), tc.get("name"), content))
...
for idx, (folder, step, name, content) in enumerate(hits):
    ...
    out_file = f"scratch/recovered_fish_{idx}.py"
