...
function applyRolePermissions(user) {
  ...
  if (user.role === 'sales' || user.role === 'tech_manager') {
    // Full access: can see and click all tabs including logs and admin
    const activeTab = document.querySelector('.nav-tab.active');
    if (!activeTab || activeTab.classList.contains('hidden')) {
      document.querySelector('[data-tab="tab-dashboard"]').click();
    }
  } else {
    // Non-admin roles cannot see logs and admin tabs
    document.querySelector('[data-tab="tab-logs"]').classList.add('hidden');
    document.querySelector('[data-tab="tab-admin"]').classList.add('hidden');
...
function setupAdminPanel() {
  ...
  const btnRestore = document.getElementById('btn-admin-restore');
  const restoreFileInput = document.getElementById('admin-restore-file-input');
  const restoreFileInfo = document.getElementById('admin-restore-file-info');
  const restoreZone = document.getElementById('admin-restore-zone');
  ...
  btnReset.addEventListener('click', async () => {
    ...
    const confirmReset = confirm('⚠️ 確定要清空並重置系統資料庫嗎？\n這將會清除所有出貨訂單與作業日誌，此動作無法復原！');
  ...
  btnBackup.addEventListener('click', () => {
    if (!currentUser) return;
    window.location.href = `/api/admin/backup?role=${currentUser.role}`;
  });

  // Restore file selection
  restoreZone.addEventListener('click', () => {
  ...
  restoreFileInput.addEventListener('change', (e) => {
    const file = e.target.files[0];
    if (file) {
      restoreFileInfo.textContent = `${file.name} (${(file.size / 1024).toFixed(2)} KB)`;
      btnRestore.disabled = false;
    } else {
      restoreFileInfo.textContent = '未選擇檔案';
  ...
  restoreZone.addEventListener('dragover', (e) => {
    e.preventDefault();
    restoreZone.style.borderColor = 'var(--accent-color, #3182ce)';
  });

  restoreZone.addEventListener('dragleave', () => {
    restoreZone.style.borderColor = '#4a5568';
  });

  restoreZone.style.cursor = 'pointer';
  ...
  btnRestore.addEventListener('click', async () => {
    ...
    const confirmRestore = confirm('確定要執行備份還原嗎？這將會覆寫目前的所有資料！');
    ...
    try {
      btnRestore.disabled = true;
      btnRestore.textContent = '還原中...';
      const res = await fetch(`/api/admin/restore?operator=${encodeURIComponent(currentUser.username)}&role=${currentUser.role}`, {
        method: 'POST',
        body: formData
      });
      const json = await res.json();
      if (json.success) {
        ...
        restoreFileInfo.textContent = '未選擇檔案';
        btnRestore.textContent = '執行資料還原';
        loadData();
        document.querySelector('[data-tab="tab-dashboard"]').click();
      } else {
        alert(`還原失敗: ${json.message}`);
        btnRestore.disabled = false;
        btnRestore.textContent = '執行資料還原';
      }
    } catch (err) {
...
// 12. Click-and-Drag Horizontal Scroll for Table
function setupDragScroll() {
  ...
  sliders.forEach(slider => {
    ...
    slider.addEventListener('mousedown', (e) => {
      // Avoid triggering drag scroll on links or interactive elements
      if (e.target.classList.contains('action-link') || e.target.tagName === 'INPUT' || e.target.tagName === 'SELECT' || e.target.tagName === 'BUTTON') {
        return;
      }
      isDown = true;
