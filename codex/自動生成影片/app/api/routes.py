...
async def parse_pdf(file: UploadFile = File(...)):
    """Parse text from PDF and generate a structured storyboard table using OpenAI."""
    try:
        # Read file contents
        ...
        # Initialize OpenAI client using configured key
        ...
        # Use gpt-4o-mini for fast text generation
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that generates clean storyboard tables in Markdown format."},
        ...
        logger.exception("PDF parsing or OpenAI prompt failed")
        raise HTTPException(
            status_code=500,
