...
async function onRequestPost(context) {
  ...
  try {
    ...
    const twTime = new Date(now.getTime() + 8 * 60 * 60 * 1e3);
    const timestamp = twTime.toISOString().replace("T", " ").substring(0, 19);
    const dataJSON = JSON.stringify(payload);
    await env.DB.prepare("INSERT INTO ipa_archives (id, timestamp, data_json) VALUES (?1, ?2, ?3)").bind(String(archiveName), timestamp, dataJSON).run();
...
async function onRequestPost2(context) {
  ...
  try {
    ...
    for (const key of Object.keys(payload)) {
      ...
      stmts.push(
        env.DB.prepare("INSERT INTO ipa_data (key, value) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value=?2").bind(key, String(value))
      );
    }
    if (stmts.length > 0) {
...
// ../.wrangler/tmp/pages-GGi4wx/functionsRoutes-0.1745657080875146.mjs
var routes = [
  {
