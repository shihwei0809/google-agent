...
def create_edu_ppt():
    ...
    add_textbox(slide2, Inches(0.6), Inches(0.4), Inches(4), Inches(0.4), "01 | 先理解目的", 16, COLOR_PRIMARY, True)
    add_textbox(slide2, Inches(0.6), Inches(0.9), Inches(10), Inches(0.8), "這份辦法要解決的，是「能力有沒有真的到位」", 38, TEXT_DARK, True)
    add_textbox(slide2, Inches(0.6), Inches(1.7), Inches(10), Inches(0.4), "訓練不只是在上課，而是從需求、執行到資格與成效的完整閉環。", 18, TEXT_DARK_MUTED, False)
    ...
    p2 = tf.add_paragraph(); p2.text = "讓員工了解公司理念、目標與作業\n流程，具備本職所需的專業知識與\n技能。"; p2.font.name = FONT_FAMILY; p2.font.size = Pt(22); p2.font.color.rgb = TEXT_DARK; p2.font.bold = True; p2.space_before = Pt(16)
    p3 = tf.add_paragraph(); p3.text = "最終目標"; p3.font.name = FONT_FAMILY; p3.font.size = Pt(14); p3.font.color.rgb = TEXT_DARK_MUTED; p3.space_before = Pt(24)
    ...
    for i, (rt, rb) in enumerate(rights):
        ...
        bar.fill.solid(); bar.fill.fore_color.rgb = COLOR_PRIMARY; bar.line.fill.background()
        add_textbox(slide2, Inches(7.1), Inches(y-0.1), Inches(5), Inches(0.4), rt, 20, TEXT_DARK, True)
        add_textbox(slide2, Inches(7.1), Inches(y+0.25), Inches(5), Inches(0.4), rb, 16, TEXT_DARK_MUTED, False)

    # 3. Flowchart
    slide3 = prs.slides.add_slide(prs.slide_layouts[6])
    set_background(slide3, BG_LIGHT)
    add_textbox(slide3, Inches(0.6), Inches(0.4), Inches(4), Inches(0.4), "02 | 管理主線", 16, COLOR_PRIMARY, True)
    add_textbox(slide3, Inches(0.6), Inches(0.9), Inches(10), Inches(0.8), "每一堂訓練，都要走完這條管理鏈", 38, TEXT_DARK, True)
    add_textbox(slide3, Inches(0.6), Inches(1.7), Inches(10), Inches(0.4), "從職能差距出發，最後回到成效與稽核。", 18, TEXT_DARK_MUTED, False)
    ...
    for i, (num, title, desc, bg_c, text_c) in enumerate(steps):
        ...
        add_textbox(slide3, Inches(x+0.2), Inches(2.8), Inches(1.8), Inches(0.4), num, 26, num_c, True)
        add_textbox(slide3, Inches(x+0.2), Inches(3.4), Inches(1.8), Inches(0.4), title, 22, text_c, True)
        desc_box = add_textbox(slide3, Inches(x+0.2), Inches(4.0), Inches(1.8), Inches(0.8), desc, 14, TEXT_DARK_MUTED if bg_c == CARD_LIGHT else TEXT_WHITE, False)
    ...
    add_textbox(slide3, Inches(0.6), Inches(6.0), Inches(12), Inches(0.5), "判斷標準：訓練完成 ≠ 上完課；必須能證明「人員受訓、能力驗證、紀錄可查」。", 20, COLOR_PRIMARY, True)

    def create_dark_content_slide(section_num, section_title, main_title, content_list):
        s = prs.slides.add_slide(prs.slide_layouts[6])
        set_background(s, BG_COLOR)
        add_textbox(s, Inches(0.6), Inches(0.4), Inches(4), Inches(0.4), f"{section_num} | {section_title}", 16, COLOR_PRIMARY, True)
        add_textbox(s, Inches(0.6), Inches(0.9), Inches(10), Inches(0.8), main_title, 36, TEXT_WHITE, True)
        
        box = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.6), Inches(1.8), Inches(12.1), Inches(5.2))
    ...
    create_dark_content_slide("05", "教育訓練設計", "講師資格與教材編選", [
        ...
        "- 教材編選與存查：",
        "  - 內訓：講師編撰或沿用公司存檔教材。",
        "  - 外訓：參加者提供教材供傳閱或列入圖書備查。"
    ])

    create_dark_content_slide("06", "教育訓練執行", "內部與外部訓練執行要點", [
        "- 內訓執行程序：",
        "  - 課前：變更工作前需受安衛訓練並填寫《紀錄表》與《報名表》。",
        "  - 課中：務必簽到《課程簽到表》。",
        "  - 課後：填寫《課後意見調查表》送總務課，受訓資料鍵入 ERP。",
        "- 外訓執行程序：",
        "  - 申請：於 QRisk 查詢需求，ERP 填寫《外派訓練申請單》。",
        "  - 結訓 10 日內：必須上系統填寫《心得報告》。",
        "  - 證照處理：法規特定證照正本留存管理部並登錄 QRisk。",
        "  - 費用與加班：非上班日可報加班，免付費亦須填申請表。"
    ])
    ...
    create_dark_content_slide("07", "考核與稽核", "資格驗證、費用與紀錄保存", [
        ...
        "  - 稽核：每年至少執行一次內部診斷，並於管理審查報告。"
    ])

    output_path = r"D:\GOOGLE ANGET\教育訓練-C0588-教材\C10200-EDU-01 教育訓練作業管理辦法(最終修復版).pptx"
