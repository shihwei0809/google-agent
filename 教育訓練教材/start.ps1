...
if (Test-Path $envFile) {
    ...
    if ($envContent -match "GEMINI_API_KEY\s*=\s*([^\r\n#]+)") {
        if ($matches[1].Trim().Length -gt 5) {
            $hasApiKey = $true
        }
    }
}
if (-not $hasApiKey) {
    Write-Host "      [Notice] GEMINI_API_KEY not set yet. Please configure backend\.env" -ForegroundColor Yellow
}

# Step 5: Start Backend
Write-Host "`n[3/4] Starting FastAPI backend (Port: 8000)..." -ForegroundColor Yellow
$backendArgs = '/k title FastAPI Backend (Port 8000) & venv\Scripts\python.exe -m uvicorn main:app --host 0.0.0.0 --port 8000'
Start-Process cmd -ArgumentList $backendArgs -WorkingDirectory $backend -WindowStyle Normal
Start-Sleep -Seconds 2

# Step 6: Start Frontend
Write-Host "[4/4] Starting React frontend (Port: 5173)..." -ForegroundColor Yellow
$frontendArgs = '/k title React Frontend (Port 5173) & npm run dev -- --host'
Start-Process cmd -ArgumentList $frontendArgs -WorkingDirectory $frontend -WindowStyle Normal
...
Write-Host "`nLauncher will close in 3 seconds..." -ForegroundColor Gray
