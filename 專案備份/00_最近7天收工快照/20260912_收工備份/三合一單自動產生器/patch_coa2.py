                ...
                for idx, row_data in enumerate(ws_l.iter_rows(values_only=True)):
                    if not headers and idx < 10:
                        ...
                        if "FinalBatchID" in row_strs:
                            headers = row_strs
                            batch_idx = headers_xxx("FinalBatchID")
                            fab_idx = headers_xxx("TSMCFab") if "TSMCFab" in headers else -1
                            phase_idx = headers_xxx("FabPhase") if "FabPhase" in headers else -1
                            date_idx = headers_xxx("DeliveryDate") if "DeliveryDate" in headers else -1
                        continue
