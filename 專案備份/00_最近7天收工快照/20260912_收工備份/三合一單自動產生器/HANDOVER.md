...
## 📅 2026-09-11 交接紀錄（未完工 / 純備份）

### ✅ 今日完成事項

| # | 功能 | 狀態 |
|---|------|------|
| 1 | 移除舊版「載入既有通知表」與「產生運輸通知表」相關 UI 與邏輯 | ✅ 完成 |
| 2 | 新增「COA 表單批次載入修改」功能 (支援檔名重組與 B6/B17 內容修改) | ✅ 完成 |

### 🔧 技術細節
- 修改 main.py 以實作新的 COA 載入邏輯。
- 透過字串替換原先存在的「載入既有通知表」等功能，減少未使用的舊代碼。
- 讀取 UI 上的 batch_var、loc_var (擷取第 2~5 碼為廠區代號)、date_var。
- 開啟選取的 COA (Excel 或 CSV) 檔案，對 B6 與 B17 進行廠區代號與日期的修改並重新儲存檔名。

### 📌 給另一台電腦（公司）的接手指示
1. 執行 git fetch --all 後切換至 eat/20260911 分支接手開發。
2. 進行相關功能測試，確認 COA 表單讀取與儲存結果符合預期。
