...
# 3. success_count update fix
content = content.replace(
    'success_count += 1\n            except Exception as e:',
    'success_count += 1\n                self.update()\n            except Exception as e:'
)

# 4. iter_rows update fix
content = content.replace(
...
# 9. on_tree_click and confirm_import
...
start_idx = content.find("def confirm_import(self):")
end_idx = content.find("# ================= 核心邏輯 =================", start_idx)
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + new_confirm + "\n\n" + content[end_idx:]

with codecs.open(r'C:\GOOGLE ANGET\三合一單自動產生器\main.py', 'w', 'utf-8') as f:
    f.write(content)
print("All patches re-applied successfully.")
