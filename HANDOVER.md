...
**今日完成進度：**
1. 成功將 main.py 的運輸通知表 openpyxl 排版邏輯完整移植到 server.py，解決網頁版產生的運輸通知表無格式問題。
2. 強化 index.html 的 Excel 匯入功能，加入圖形化預覽 Modal，並修正欄位讀取錯位問題 (出貨日期 B, 批號 C, 地點 E)。
3. 新增「📅 優先抓今天~後天」按鈕，實作智慧日期過濾功能。
4. 加入瀏覽器端 Tesseract.js AI 辨識機制，解決公司內網阻擋 OCR 安裝檔的問題，實現 COA 截圖精準裁切 (單行標頭+專屬資料列)。
5. 支援讀取 Excel 檔案內所有的分頁資料，不再侷限於第一個分頁。
6. 修正 onPasteHandler，支援略過槽號並自動解析複製貼上的日期格式。
7. **修復「載入既有運輸通知表」舊快取覆蓋 Bug**：移除了退回讀取根目錄舊快取 `last_generated_session.json` 的邏輯，改為 100% 精準解析所選取的 `運輸通知表.xlsx` 每一筆排程卡片（槽號、地點、日期、時間）。
8. **工具列排版重構與滑鼠滾輪支援**：將工具列重構為左右分區（左側：新增10列、清除全部；右側：COA截圖、載入通知表、Excel匯入），解決按鈕被擠壓看不見的問題，並加入全域滑鼠滾輪上下滾動支援。
9. **網頁版 ZIP 同步寫入 session.json**：網頁版產出的壓縮檔同步包含 session.json，確保跨本機與網頁版資料互通。

**避坑指南與交接注意事項：**
- **Session 快取避坑**：載入運輸通知表 Excel 時，只有在 Excel 檔案所在資料夾存在 `session.json` 時才能讀取，嚴禁退回到根目錄讀取歷史全域 session，否則會覆蓋使用者當前選擇的檔案。
- **OCR 限制**：由於目標主機封鎖 tesseract.exe，系統已改由前端 index.html 透過 CDN 載入 Tesseract.js 進行影像辨識。請勿嘗試在後端 server.py 中引入需要安裝執行檔的 OCR 套件。
- **日期格式**：SheetJS 讀取出的日期格式可能隨 Excel 儲存格設定變化 (如 8/18 或 yyyy-mm-dd)，目前已在 JS 端加入完整的正規表示式備援解析，請勿輕易修改。
- **🚨 專案獨立鐵律**：鴻勝系統（`三合一單網頁架機伺服器`）與勝一系統（`勝一三合一單產生系統`）業務與批號規則完全獨立（鴻勝批號嚴格限 10 碼，勝一為 10~11 碼與多產品）。**嚴禁 AI 擅自將兩者規則或邏輯結合，若有任何結合需求必須先主動詢問使用者獲得明確同意。**
- **預先驗證機制**：所有對 Excel 或後端資料庫的大規模寫入或更新操作，開工時必須先進行「預檢測試」，確保測試環境與執行目標目錄路徑（Path）與權限正確，嚴禁在未經檢查的情況下直接於正式目錄中執行自動化腳本。
- **大廳資料庫安全分離與防破壞規範**：
  - 過去曾因腳本使用危險的正則表達式嘗試覆寫 `說明書/index.html` 內嵌變數，導致大廳首頁卡片被破壞清空。
  - **正確且永久的做法**：大廳資料一律透過外掛 `manuals_db.js` 載入，`compile_manuals.py` 只寫入 `manuals_db.js`，**絕對不修改 `index.html` 的程式碼結構**。修改任何重要邏輯前必須先在沙盒或本地驗證無語法錯誤後方可生效。
...
### 📌 全新建立獨立專案：勝一三合一單產生系統 (勝一三合一單產生系統\)
* **建立背景**：因應勝一化工特定訂單格式（勝一訂單.xlsx），以原三合一單系統為母版，建立全新獨立子專案資料夾與獨立產出路徑（勝一三合一單輸出_YYYYMMDD），互不干擾。
* **完成核心功能**：
  1. **勝一訂單專屬解析與台積電精準過濾**：自動識別「空白班表」分頁，依對象簡稱與指送地智慧過濾排除非台積電廠商（如南亞、長春），精準提取 304 筆純台積電出貨排程。
  2. **「當天至 +2 天」智慧優先推薦預選**：自動計算系統「今天~後天 (D ~ D+2)」3 天排程區間並高亮預選，提供快捷按鈕切換「今天」、「明天」、「後天」或倒數筆數。
  3. **多產品品名辨識與固定重量自動對照**：自動擷取訂單品名（SEP73E5, PMAHQ, SEP73E4, CPNE4R, IPA HQ 等），運輸通知表標題動態顯示 Shiny [品名] Lorry，充填數量由內部 PRODUCT_WEIGHT_MAP 自動對照填入，不需現場人員手動輸入。
  4. **勝一 10~11 碼批號與槽號自動提取**：支援 10~11 碼批號（如 26707S125T1），自動提取槽號（如 S125, S96, S405）並納入檔名防呆防覆蓋。
  5. **指送地智慧正規化**：自動清理「台積12廠P1」➔「12P1」、「台積新竹14廠-P5」➔「14P5」、「台積先進AP8」➔「AP8」，自動匹配長代號。
  6. **COA 檢驗智慧 OCR 辨識**：支援一鍵上傳檢驗截圖，自動辨識批號裁切表頭與數據列嵌入三合一單 F5 儲存格。
  7. **產出專屬操作手冊 (Word + PDF)**：產出零 Emoji、含 7 大 FAQ 與高解析標註截圖之最新操作手冊。
* **避坑與修復日誌 (Bug Fixes)**：
  - **.bat 閃退修復**：PowerShell UTF-8 BOM 導致 cmd.exe 巢狀括號解析出錯，已全面重構為純 ASCII 標準 Delayed Expansion 迴圈架構。
  - **UnboundLocalError (re / os)**：函式內部存在局部 import re / import os 導致 Python 變數遮蔽，已全面統一移至模組最頂層全域引入。
  - **KeyError (prod_var / weight_var) & 欄位對齊**：修正動態列 dd_input_rows 與 UI 11 欄 Grid 垂直對齊，移除重量欄位並清理殘留字典鍵調用。
  - **手冊圖文精準對齊**：重新繪製 image_shinyi_import.png，在圖 5 中真實呈現「當天至+2天」紅框聚焦推薦介面。
