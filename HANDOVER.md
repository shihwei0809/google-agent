...
### 📅 2026-08-20 交接紀錄 (溫度通報系統 - 本機斷線偵測與個人通知)

**今日完成進度與修復：**
1. **修復本機排程中斷 (最後心跳停留在 7/16)**：
   * **問題原因**：專案路徑由舊的 `c:\GOOGLE ANGET\溫度通報\` 改為 `c:\GOOGLE ANGET\第二類_生產管理與API串接\溫度通報\`。公司本機的 Windows 排程器仍指向舊的路徑，導致背景程式無法執行。
   * **解決方式**：已將已測試完成的 Python 與 Google Apps Script (GAS) 程式碼複製並覆蓋至正確的 `第二類_生產管理與API串接\溫度通報` 中。請在回公司電腦後，對 `建立本機排程.bat` 按右鍵以「系統管理員身分」執行一次，即可修正排程路徑恢復心跳。
2. **新增本機斷線個人 LINE / Email 通知功能**：
   * 在雲端 `Code.gs` 與 `複製雲端代碼.py` 中的 `checkWeatherAndNotify` 實作了「本機斷線警報」與「本機恢復通知」。
   * 當本機超過 15~20 分鐘未回報心跳，雲端會發送 Email 給 `shihwei0809@gmail.com`，並透過 LINE 自動過濾出開頭為 `U` 的個人 User ID 發送警報（過濾掉 `C` 與 `R` 開頭的群組和聊天室），確保只通知您個人，不打擾群組人員。本機恢復運作後亦會發送恢復通知。

**Git 備份與接手指示**：
* 由於本地與遠端歷史紀錄分支分叉（有 374 個未拉取提交），為防強行合併弄亂其他專案，本次修改已成功推送至新分支 `feat/offline-alert`，回公司電腦後，請拉取此分支並合併。

### 2026-08-24 交接紀錄
- **完成事項**：完成「資材課」2026 下半年度價值型 KPI 擬定 (結合化學業特性、AI 預警、建廠專案及 ESG 綠色採購)。
- **避坑指南**：本次僅進行 KPI 規劃諮詢，無修改程式碼，無發生系統錯誤。
