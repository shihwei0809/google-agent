...
def patch_main(path):
    ...
    row_repl = r'''\1
            ...
            part_entry.grid(row=row_grid_idx, column=9, padx=2, pady=2, sticky="ew")\2\3\g<4>10\g<5>'''
    ...
    for prefix in ['entry', 'self.entries[r_idx]']:
        ...
        content = content.replace(clear_pattern, clear_repl)
    ...
    content = content.replace(trace_pattern, trace_repl)
    ...
    parse_logic_pattern = r'(# 3\. 到貨地.*?continue)'
    parse_logic_repl = r'''\1

            # 4. 出貨區
            if not res.get("origin") and any(k in p_upper for k in ["崙尾", "彰濱", "L1", "L2", "廠", "區"]):
                res["origin"] = p
                continue'''
    # Wait, the Chinese chars might be encoded weirdly in the regex, let's use a simpler anchor.
    # Actually, we can just append it before "return res".
    ret_res_pattern = r'return res\n\n    def on_paste'
    ...
    on_paste_repl = r'''elif parsed["loc"]:
    ...
    # 8. load_mapping
    load_pattern = r'self\.mapping_dict = \{\}'
    load_repl = r'self.mapping_dict = {}\n        self.part_mapping_dict = {}'
    # We only want to replace the first occurrence in load_mapping.
    # Better to rewrite load_mapping safely.
