...
- **收工備份規範**：當收到「收工備份」或「放工備份」指令時，除了同步本地程式至 Google 雲端硬碟外，必須**自動檢查並確認相關的說明書與操作手冊是否已隨本次開發完成更新**。最後自動執行 Git 提交（git commit）並上傳（git push）至 GitHub 專案庫，確保雲端硬碟與 GitHub 上的程式與說明文件皆為最新版本。
