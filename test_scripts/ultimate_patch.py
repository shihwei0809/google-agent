...
"""
    if 'def _extract_batch_from_coa_bytes' not in text:
        text = text.replace('def extract_tank_from_batch', func + 'def extract_tank_from_batch')

    # 2. Fix imports and loop
    ...
    for i, line in enumerate(lines):
        ...
        if 'prefix = base_name[:idx]' in line and i+1 < len(lines) and 'suffix = base_name[idx + len(matched_batch):]' in lines[i+1]:
            continue
        if 'suffix = base_name[idx + len(matched_batch):]' in line and i-1 >= 0 and 'prefix = base_name[:idx]' in lines[i-1]:
            continue
...
full_patch(r'd:\GOOGLE ANGET\三合一單網頁架機伺服器\server.py')
full_patch(r'd:\GOOGLE ANGET\勝一三合一單網頁架機伺服器\server.py')
